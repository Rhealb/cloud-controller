...
const (
	...
	//save configmap in slbNamespace
	slbNamespace  = "kube-system"
	defaultConfig = "alicloud-ali-slb"
)
...
func (s *ServiceController) ensureLoadBalancer(service *v1.Service, cachedService *cachedService) (*v1.LoadBalancerStatus, error) {
	...
	serviceValue, err := s.getServiceValueFromConfigmap(service)
	if err != nil {
		return nil, err
	}
	glog.Infof("origin service info is %v,will be used in EnsureLoadbalancer", serviceValue)

	// - Only one protocol supported per service
	// - Not all cloud providers support all protocols and the next step is expected to return
	//   an error for unsupported protocols
	status, err := s.balancer.EnsureLoadBalancer(s.clusterName, service, nodes, serviceValue)
	if err != nil {
		return nil, err
	}
...
func (s *ServiceController) getServiceValueFromConfigmap(service *v1.Service) (serviceValue string, err error) {
	slbcm, err := s.getLoadbalancerConfigMap(service)
	if err != nil && !errors.IsNotFound(err) {
		return "", err
	}
	if slbcm == nil || slbcm.Data == nil {
		return "", nil
	}

	serviceKey, _ := alicloud.GetServiceTag(service)

	for k, v := range slbcm.Data {
		if k == serviceKey {
			return v, nil
		}
...
func (s *ServiceController) syncService(key string) error {
	...
	default:
		...
		if needLoadBalancer(service) {
			updateServiceErr := s.updateServiceInConfigmap(service)
			if err != nil {
				glog.Errorf("update service in configmap failed,%v", err)
...
func (s *ServiceController) processLoadBalancerDelete(cachedService *cachedService, key string) (error, time.Duration) {
	...
	s.eventRecorder.Event(service, v1.EventTypeNormal, "DeletingLoadBalancer", "Deleting load balancer")
	err := s.balancer.EnsureLoadBalancerListenersDeleted(s.clusterName, service)
	if err != nil {
	...
	//delete configmap
	deleteSlb, err := s.deleteServiceInConfigmap(service)
	if err != nil {
		glog.Errorf("failed to delete service info in configmap,%v,%v", service, err)
		return err, cachedService.nextRetryDelay()
	}
	if deleteSlb {
...
func (s *ServiceController) updateServiceInConfigmap(service *v1.Service) error {
	slbcm, err := s.getLoadbalancerConfigMap(service)
	if err != nil && !errors.IsNotFound(err) {
		return err
	}
	serviceKey, serviceValue := alicloud.GetServiceTag(service)
	loadbalancerConfigName := s.getConfigmapName(service)
	if slbcm == nil {
		slbcm = &v1.ConfigMap{
			ObjectMeta: meta_v1.ObjectMeta{
				Namespace: slbNamespace,
		...
		glog.Infof("create data in configmap,key:%v,value:%v", serviceKey, serviceValue)
		_, err = s.kubeClient.CoreV1().ConfigMaps(slbNamespace).Create(slbcm)

		return err

	}
	if slbcm.Data == nil {
		slbcm.Data = make(map[string]string)
	}

	// update or add service value
	slbcm.Data[serviceKey] = serviceValue

	glog.Infof("update data in configmap,key:%v,value:%v", serviceKey, serviceValue)
	_, err = s.kubeClient.CoreV1().ConfigMaps(slbNamespace).Update(slbcm)

	return err

}

func (s *ServiceController) deleteServiceInConfigmap(service *v1.Service) (deleteSlb bool, err error) {
	slbcm, err := s.getLoadbalancerConfigMap(service)
	if err != nil {
		return false, err
	}
	if slbcm == nil {
		glog.Infof("configmap could not be found")
		return false, nil
	}
	if slbcm.Data == nil && slbcm.ObjectMeta.Name != defaultConfig {
		err = s.kubeClient.CoreV1().ConfigMaps(slbNamespace).Delete(slbcm.ObjectMeta.Name, nil)
		if err != nil {
			return false, err
		}
		return true, nil
	...
	for k, _ := range slbcm.Data {
		if k == serviceKey {
			glog.Infof("find service %s in configmap, will delete it", serviceKey)
			if len(slbcm.Data) == 1 && slbcm.ObjectMeta.Name != defaultConfig {
				err = s.kubeClient.CoreV1().ConfigMaps(slbNamespace).Delete(slbcm.ObjectMeta.Name, nil)
				if err != nil {
					return false, err
				}
...
func (s *ServiceController) getConfigmapName(service *v1.Service) string {
	return "alicloud-" + s.loadBalancerName(service)
}

//return the configmap name of the loadbalancer
func (s *ServiceController) getLoadbalancerConfigMap(service *v1.Service) (configmap *v1.ConfigMap, err error) {
	loadbalancerConfigName := s.getConfigmapName(service)
	slbcm, err := s.kubeClient.CoreV1().ConfigMaps(slbNamespace).Get(loadbalancerConfigName, meta_v1.GetOptions{})
	if err != nil {
		return nil, err
	}
	return slbcm, nil

...
func (f *ListenerManager) Apply(originServicePort string) error {
	originalPortMap := convertSlbTagToMap(originServicePort)
	//newPortMap := convertSlbTagToMap(newServicePort)
	if err := f.Deletions(originalPortMap); err != nil {
		return err
	}
	return f.Updations(originalPortMap)
}

func (f *ListenerManager) Updations(portMap map[string]bool) error {
	...
	for _, spec := range f.service.Spec.Ports {
		...
		for _, back := range pprot.ListenerPortAndProtocol {
			...
			if _, ok := portMap[strconv.Itoa(back.ListenerPort)]; !ok {
				continue
			}
...
func (f *ListenerManager) Deletions(portMap map[string]bool) error {
	anno := serviceAnnotation(f.service, ServiceAnnotationLoadBalancerProtocolPort)
	pprot := f.loadbalancer.ListenerPortsAndProtocol
	glog.Infof("delete svc in portMap %v", portMap)

	//	f.loadbalancer
	for _, back := range pprot.ListenerPortAndProtocol {
		glog.Infof("range back.ListenerPort %v", back.ListenerPort)
		if _, ok := portMap[strconv.Itoa(back.ListenerPort)]; !ok {
			glog.Infof("not find back.ListenerPort %v", back.ListenerPort)
			continue
		}
		...
		for _, spec := range f.service.Spec.Ports {
			if int(spec.Port) == back.ListenerPort {
				...
				if strings.ToUpper(back.ListenerProtocol) == strings.ToUpper(realProtocol) {
					found = true
					glog.Infof("deletion found in service: %v", spec.Port)
					break
				}
			}
		}

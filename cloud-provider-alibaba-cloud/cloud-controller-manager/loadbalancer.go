...
func (s *LoadBalancerClient) EnsureLoadBalancer(service *v1.Service, nodes []*v1.Node, vswitchid string, originServiceInfo string) (*slb.LoadBalancerType, error) {
	...
	if (!isUserDefinedLoadBalancer(request)) ||
		...
		err = NewListenerManager(s.c, service, origined).Apply(originServiceInfo)
		if err != nil {
			return nil, err
		}
	}
...
func (s *LoadBalancerClient) EnsureLoadBalanceListenersDeleted(service *v1.Service) error {
	...
	_, serviceTagValue := GetServiceTag(service)
	portMap := convertSlbTagToMap(serviceTagValue)
	for portString, _ := range portMap {
		port, err := strconv.Atoi(portString)
		if err != nil {
			return err
		}
		err = s.c.DeleteLoadBalancerListener(lb.LoadBalancerId, port)
		if err != nil {
			return err
		}
	}

...
//load balancer name
const ServiceAnnotationAliLoadBalancerName = "enndata.io/ali-load-balancer-name"
...
func GetLoadBalancerName(service *v1.Service) string {
	if service.Annotations == nil {
		//return OriginalGetLoadBalancerName(service)
		return "ali-slb"
	}

	//return service annotation loadbalancer specify name
	if service.Annotations[ServiceAnnotationAliLoadBalancerName] != "" {
		return service.Annotations[ServiceAnnotationAliLoadBalancerName]
	}

	//Todo: do not consider about internal lb

	internalAnnotation := service.Annotations[ServiceAnnotationLoadBalancerAddressType]
	if internalAnnotation == "intranet" {
		//internal lb name
		return "ali-slb-intranet"
	}
	return "ali-slb"
}
...
func OriginalGetLoadBalancerName(service *v1.Service) string {
...
type LoadBalancer interface {
	...
	// Parameter 'clusterName' is the name of the cluster as presented to kube-controller-manager
	EnsureLoadBalancer(clusterName string, service *v1.Service, nodes []*v1.Node, originServiceValue string) (*v1.LoadBalancerStatus, error)
	...
	// Implementations must treat the *v1.Service parameter as read-only and not modify it.
	// Parameter 'clusterName' is the name of the cluster as presented to kube-controller-manager
	EnsureLoadBalancerListenersDeleted(clusterName string, service *v1.Service) error
